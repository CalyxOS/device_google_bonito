...
#define LOG_TAG "android.hardware.usb-service.bonito"
...
Status queryMoistureDetectionStatus(std::vector<PortStatus> *currentPortStatus) {
    ...
    if (currentPortStatus == NULL || currentPortStatus->size() == 0) {
        ALOGE("currentPortStatus is not available");
        return Status::ERROR;
    }
    ...
    (*currentPortStatus)[0].contaminantDetectionStatus = ContaminantDetectionStatus::NOT_DETECTED;
    (*currentPortStatus)[0].supportsEnableContaminantPresenceDetection = false;
